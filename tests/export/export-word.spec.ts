...
test('Export > Word triggers a .docx file download', async ({ page }) => {
  await login(page);
  ...
  // Click Word option
  await page.locator('.export-menu').locator('text=/Word|\.docx/i').first().click();

  const download = await downloadPromise;
  const filename = download.suggestedFilename();
  expect(filename.toLowerCase()).toContain('.docx');
});
